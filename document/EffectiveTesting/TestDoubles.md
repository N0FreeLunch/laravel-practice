...
곧, Mock은 검증 대상과의 적절한 상호작용을 이루는 대상을 생성하기 위해서 테스트 코드의 통과 과정을 거쳐가며 생성되는 대상이다.

### Fake

실제로 사용하는 것을 대신하기 위해 사용하는 것으로, 외부 서비스, 프로그램, 거대하거나 복잡한 로직을 직접 사용하기 어려운 상황에서 이를 단순화 또는 간략화를 하거나 빠른 속도를 가진 대상으로 대체해서 로직의 동작을 체크하기 위해서 사용하는 대상이다.

Fake 자체는 결점이 있더라도 검증하려는 대상과의 상호 작용에 있어서는 거의 대부분의 호환성을 제공하여 검증 대상의 동작을 검증할 수 있게 만들어진다.
